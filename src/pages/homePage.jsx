...
class HomePage extends Component {
  ...
  async componentDidMount() {
    let isLoggedIn =
      this.props.isLoggedIn || sessionStorage.getItem("isLoggedIn");
    if (Boolean(this.props.location.state)) {
      await this.setState({
        data_pag: {
          ...this.data_pag,
          url: issue_url + `?tags=${this.props.location.state.tag.id}&page=1`,
        },
        tag: this.props.location.state.tag,
      });
    }
    let res = await fetch(this.state.data_pag.url, {
      headers: { Authorization: `Token ${sessionStorage.getItem("token")}` },
    });
    let data = await res.json();
    await this.setState({
      data: data.results,
      data_pag: { ...this.state.data_pag, count: data.count },
  ...
  async componentDidUpdate(prevprops, prevstate) {
    ...
    if (this.props.location !== prevprops.location) {
      ...
      if (current_tag_state) {
        ...
        await this.setState({
          data_pag: { ...this.data_pag, url: issue_url + `?page=1` },
        });
      }
      ...
      <Container fluid className="ContainerDiv">
        <Container>
          <Breadcrumb size="huge" className="PageTopic">
            <Breadcrumb.Section>
              <Header>
                {!Boolean(this.state.tag)
                  ? "Issues"
                  : `Issues tagged [${this.state.tag.name}]`}
              </Header>
            </Breadcrumb.Section>
          </Breadcrumb>
          <Header dividing />
          <IssueFilter
            url={this.state.data_pag.url}
            get_content={this.get_content}
          />
          <PaginationContainer
            onPageChange={(data, url) => {
              this.setState({
                data: data,
                data_pag: { ...this.state.data_pag, url: url },
              });
            }}
            data_pag={this.state.data_pag}
          />
          <Container>{this.ListCards()}</Container>
          <PaginationContainer
            onPageChange={(data, url) => {
              this.setState({
                data: data,
